...
const { getMarkdown, getDocument, readFile } = require('./util');
...
describe('public api', () => {
  ...
  test('slug', async () => {
    ...
    const indexPage = await setupPage(
      { filePath: indexPageFixture.filePath, relative: indexPageFixture.relative },
      { markdown }
    );
    ...
    const page = await setupPage(
      { filePath: pageFixture.filePath, relative: pageFixture.relative },
      { markdown }
    );
