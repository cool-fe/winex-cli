...
module.exports = class DevProcess extends EventEmitter {
  constructor(context) {
    ...
    this.context.options.cache = false;
  }
  ...
  watchSourceFiles() {
    ...
    this.pagesWatcher = chokidar.watch(['**/*.md', '.vuepress/components/**/*.vue'], {
  ...
  watchUserConfig() {
    this.watchFiles = [
      ...
      (this.context.siteConfig.extraWatchFiles || []).map((file) =>
        normalizeWatchFilePath(file, this.context.sourceDir)
      )
    );
  ...
  async resolveHost() {
    const { host, displayHost } = await resolveHost(
      this.context.options.host || this.context.siteConfig.host
    );
    this.host = host;
