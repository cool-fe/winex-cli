...
import { moduleResolver, datatypes, logger, chalk, normalizeConfig } from '../../shared-utils';
import instantiateOption from './override/instantiateOption';
import { flattenPlugin } from './util';
import { PLUGIN_OPTION_MAP } from './constants';

const { getPluginResolver } = moduleResolver;
const { assertTypes, isPlainObject } = datatypes;
...
export default class PluginAPI {
  ...
  private _pluginQueue: any[];

  private _loggedPlugins: any[];

  private _initialized: boolean;

  private _pluginResolver: any;
  ...
  normalizePlugin(type: string, pluginRaw: any, pluginOptions = {}) {
  ...
  useByPluginsConfig(pluginsConfig: any) {
    // eslint-disable-next-line no-param-reassign
  ...
  registerOption(key: string | number, value: undefined, pluginName: any) {
    const option = PLUGIN_OPTION_MAP[key];
  ...
  applySyncOption(name: any, ...args: any[]) {
  ...
  async applyAsyncOption(name: any, ...args: any[]) {
...
function pluginLog(name: any, shortcut: any) {
  return shortcut
