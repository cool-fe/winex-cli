...
export async function installSaveDev(
  ...
  return new Promise(async (resolve) => {
    ...
    const cliArgus = pmToolName === 'yarn' ? ['add', '--dev'] : ['install', '--save-dev'];
    try {
 ...
 */
export function fetchPeerDependencies(packageName: string): any {
  const npmProcess = spawn.sync('npm', ['show', '--json', packageName, 'peerDependencies'], {
    encoding: 'utf8'
  });
 ...
 *                               and values are booleans indicating installation.
 */
export function checkDeps(packages: string[], rootDir: any): any {
 ...
 *                               and values are booleans indicating installation.
 */
export function checkDevDeps(packages: string[]): any {
 ...
 */
export function checkPackageJson(startDir: string): boolean {
