 ...
 */
async function configEslintRC(projectType: string, supportTypeScript: boolean): Promise<void> {
  ...
  if (checkResult.length) {
    ...
    if (answer.eslint) {
      ...
      let newFileJSON = {};
      checkResult.forEach(async (oldFile) => {
      ...
      Logger.info(chalk.yellow(`👏 eslint配置更新完成, please check for sure. \n`));
